import { Button, Container, Paper, PasswordInput, TextInput, Title, useForm } from "@quassel/ui";
...
function Session() {
  ...
  const createSessionMutation = $api.useMutation("post", "/session", {
    onSuccess: (data) => {
      $session.set({ email: data.email, role: data.role, expiresAt: data.expiresAt.toString() });
